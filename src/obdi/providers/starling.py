...
**Internal transfers appear in the parent feed.** Movements between your own
Spaces surface with `source` of `INTERNAL_TRANSFER` and double-count against
spending unless dropped.
"""
...
def fetch_categories(
    ...
    """Every category holding transactions: the default plus one per Space.
    ...
    for goal in payload.get("savingsGoals", []):
        if goal.get("savingsGoalUid"):
            categories.append(goal["savingsGoalUid"])
    return categories
...
def to_transaction(item: dict, *, account_id: str) -> Transaction | None:
    ...
    Returns None for movements that never happened (declined, account checks)
    and for transfers between your own Spaces, which the parent feed reports
    but which are not spending.
    """
    if item.get("source") == INTERNAL_SOURCE:
        return None
