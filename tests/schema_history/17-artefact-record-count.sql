-- The schema as it SHIPPED at 6ea711e - artefacts carry their record
-- count, and the raw_artefacts key still carries the ORIGIN. This is the
-- shape the live store was found in: 62 artefact rows for 31 documents,
-- each landed twice because a folder upload names a file by its path and
-- the file picker names it bare.
